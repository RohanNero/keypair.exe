...
const sign = async (privKey, msgHash) => {
  ...
  ${colors.cyan}║${colors.reset}   ${colors.yellow}✍️     MESSAGE SIGNING    ✍️${colors.reset}      ${colors.cyan}║${colors.reset}
  ${colors.cyan}╚═══════════════════════════════════╝${colors.reset}
  ...
  ${colors.yellow}recovery:${colors.reset}${colors.green} ${signature.recovery}${colors.reset}
...
rl.question("Enter private key: ", (privKey) => {
  rl.question("Enter message hash: ", (msgHash) => {
    if (!privKey || !msgHash) {
      console.error(`${colors.red}Error: Both private key and message hash are required.${colors.reset}`);
      rl.close();
    } else {
